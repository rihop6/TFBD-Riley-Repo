...
SELECT DISTINCT Quantity FROM [Production].[ProductInventory];
...
-- If you/someone meess up the datetime formats
-- ETL Pipeline: SQL (local time) -> DATA LAKE (Datetime with offset)
...
-- Edge Cases: Leap Years, Last Day of Month, February (find answers in docs)

-- EOMONTH (end of month)
SELECT TOP 3 order_date, EOMONTH(order_date) FROM orders;

-- Convert our date to the datetime with offset
SELECT GETDATE(), TODATETIMEOFFSET(GETDATE(), '-04:00');
...
A quick note on Collation (encoding)
	- If I input a string - result will use the input string's collation
...
-- SUBSTRING() (remember SQL is 1 indexed)
...
-- LTRIM, RTRIM, TRIM
SELECT
	LTRIM('		Hello World		') AS [Left Trim],
...
CREATE TABLE users (
    ...
    user_role VARCHAR(30) DEFAULT 'U', 
...
VALUES ('Sora', 'Hearts', 'keyblade@master.com', '019he211', 'U', 1);
...
-- SELECT o.status, COALESCE(date_returned, date_delivered, date_shipped, warehouse_proceed, order_placed) AS [Date]
...
CASE order_item_subtotal
	WHEN order_item_subtotal >= 200.0 THEN 'expensive'
	WHEN order_item_subtotal < 200.0 THEN 'cheap';
/*
...
- View does NOT physicall store the data
...
-- Functionally they work as a subquery
-- WITH <name> AS
...
-- CTEs don't exist past the semicolon- views stay
SELECT * FROM orders_v;
SELECT * FROM order_details_nq;
...
-- Columns for the specified table need to match the query results
...
ALTER TABLE custom_order_metrics
...
-- This shows how many rows we're scanning
SELECT COUNT(1)
FROM orders o
JOIN order_items oi ON o.order_id = oi.order_item_order_id;
...
-- Returning a table is Magnitudes Faster than running a query
SET SHOWPLAN_ALL ON
